...
void loadData(const char* filename, sdds::Autoshop& as)
{
	...
	while (file)
	{
		...
		catch (std::invalid_argument& type) {
			std::cerr << "Unrecognized record type: [" << type.what() << "]" << std::endl;
		}
		catch (std::exception&) {
...
int main(int argc, char** argv)
{
	...
	std::list<const sdds::Vehicle*> vehicles;
	{
		...
		auto fastVehicles = [](const sdds::Vehicle* veh) { return veh->topSpeed() > 300 ? true : false; };
	...
	std::cout << std::endl;
	{
		...
		auto brokenVehicles = [](const sdds::Vehicle* veh) { return veh->condition() == "broken" ? true : false; };
